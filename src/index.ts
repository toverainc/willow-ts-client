...
export class WillowClient extends (EventEmitter as new () => TypedEmitter<WillowClientEvents>) {
    ...
    async mute(mute: boolean) {
        ...
        await this.sender.replaceTrack(mute ? null : this.track || null)
    }
    ...
    private async negotiate(attempts: number = 1, attemptBackoff: number = 5000) {
        ...
        for (let attempt = 1; attempt <= attempts; attempt++) {
            ...
            this.emit('onLog', `localDescription offer ${JSON.stringify(pc.localDescription, null, 2)}`);
            //pc.setLocalDescription({type: 'offer', sdp:sdpFilterCodec(pc.localDescription?.sdp)});
            ...
            try {
                ...
                answer = await (await fetch(`${this.config.host}`, {
                    method: 'POST',
                    body: JSON.stringify({
